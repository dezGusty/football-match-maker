...
export class PlayerDashboardComponent implements OnInit {
    ...
    async loadPlayerData() {
        ...
        if (userId) {
            try {
                const userResponse = await fetch(`http://localhost:5145/api/user/${userId}`);
                if (userResponse.ok) {
    ...
    async loadMatches() {
        ...
        try {
            ...
            this.upcomingMatches = futureMatches.filter(match =>
                match.playerHistory?.some(ph => ph.player.id === this.currentPlayer?.id)
            );
    ...
    getPlayerTeamName(match: Match): string {
        const playerHistory = match.playerHistory?.find(ph => ph.player.id === this.currentPlayer?.id);
